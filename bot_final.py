...
def fetch_ivasms_messages():
    ...
    try:
        ...
        # --- خطوة 1: Get SMS (الضغط على زر Get SMS) ---
        print(f"[iVasms] 🔴 جلب المجموعات...")
        step1 = session.post(
            "https://www.ivasms.com/portal/sms/received/getsms",
            headers=headers,
            data={'from': start_date, 'to': end_date, '_token': csrf_token},
            timeout=30
        )
        soup1 = BeautifulSoup(step1.text, 'html.parser')

        # استخراج أسماء وأرقام المجموعات من الجدول
        group_ids = []
        for row in soup1.find_all('tr'):
            cells = row.find_all('td')
            if cells:
                # أول td فيه اسم المجموعة، نبحث عن رابط أو onclick
                onclick = row.get('onclick', '') or (cells[0].get('onclick', '') if cells else '')
                match = re.search(r"'([^']+)'", onclick)
                if match:
                    group_ids.append(match.group(1))

        # إذا ما لقينا من onclick، جرب استخراج من النص مباشرة
        if not group_ids:
            # ابحث عن أي رقم مجموعة في الصفحة
            for tag in soup1.find_all(onclick=True):
                match = re.search(r"[\'\"]([0-9]+)[\'\"]", tag.get('onclick', ''))
                if match:
                    group_ids.append(match.group(1))

        # إذا لا يزال ما في، استخدم المجموعة المعروفة 9244
        if not group_ids:
            print(f"[iVasms] ℹ️ استخدام المجموعة الافتراضية 9244")
            group_ids = ['9244']
        ...
        for group_id in group_ids:
            ...
            step2 = session.post(
                ...
                headers=headers,
                data={'start': start_date, 'end': end_date, 'range': group_id, '_token': csrf_token},
                timeout=30
            )
            ...
            # إذا ما لقينا أرقام من الخطوة 2، استخدم الكومبو
            if not phone_numbers:
                print(f"[iVasms] ℹ️ استخدام أرقام الكومبو...")
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
                c.execute("SELECT numbers FROM combos")
                rows = c.fetchall()
            ...
            print(f"[iVasms] 📱 عدد الأرقام: {len(phone_numbers)}")
            ...
            for phone in phone_numbers:
                try:
                    ...
                    step3 = session.post(
                        ...
                        headers=headers,
                        data={'start': start_date, 'end': end_date, 'Number': phone, 'Range': group_id, '_token': csrf_token},
                        timeout=15
                    )
