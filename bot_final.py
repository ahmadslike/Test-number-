...
def send_welcome(message):
    ...
    fancy_text = (
        "<b>❍<u>𝒀𝑶𝑼𝑺𝑺𝑬𝑭 • 𝑱𝑶𝑶 𝒃𝒐𝒕 </u>❍</b>\n\n"
        "<b>🔋 <u>مـنـور بـوت • 𝑱𝑶𝑶 • يـحـب</u></b>\n\n"
        "<b>🎓 <u>الـمـطـور</u>  • <a href='tg://user?id=8594366256'>𝑱𝑶𝑶</a></b>\n\n"
        "<b>────────────────────</b>\n"
...
def login_to_ivasms():
    try:
        ...
        login_page = session.get("https://www.ivasms.com/login", timeout=30)
        ...
        login_resp = session.post("https://www.ivasms.com/login", data=login_data, timeout=30, allow_redirects=True)

        # تحقق من نجاح الدخول
        if "login" not in login_resp.url.lower() or "dashboard" in login_resp.url.lower() or "portal" in login_resp.url.lower():
            print(f"[{dash['name']}] ✅ تسجيل الدخول ناجح: {login_resp.url}")
            # جلب CSRF من الصفحة الحالية
            page_soup = BeautifulSoup(login_resp.text, 'html.parser')
            csrf_meta = page_soup.find('meta', {'name': 'csrf-token'})
            if csrf_meta:
                dash['csrf_token'] = csrf_meta.get('content')
            else:
                # جرب جلب صفحة SMS مباشرة
                sms_page = session.get("https://www.ivasms.com/portal/sms/received", timeout=30)
                sms_soup = BeautifulSoup(sms_page.text, 'html.parser')
                csrf_meta = sms_soup.find('meta', {'name': 'csrf-token'})
                if csrf_meta:
                    dash['csrf_token'] = csrf_meta.get('content')
            dash['is_logged_in'] = True
            return True
        else:
            print(f"[{dash['name']}] ❌ فشل تسجيل الدخول، URL: {login_resp.url}")
            return False
    except Exception as e:
    ...
    try:
        ...
        for phone in all_numbers:
            try:
                ...
                # جرب بدون Range وبدون _token أيضاً
                for payload in [
                    {'start': start_date, 'end': end_date, 'Number': clean_phone, '_token': csrf_token},
                    {'from': start_date, 'to': end_date, 'number': clean_phone, '_token': csrf_token},
                    {'start': start_date, 'end': end_date, 'Number': clean_phone},
                ]:
                    resp = session.post(sms_url, headers=headers, data=payload, timeout=15)
                    
                    # طباعة للتشخيص
                    if len(resp.text) > 50:
                        print(f"[iVasms] 📄 رد {clean_phone}: {resp.text[:200]}")
                    ...
                    # طريقة 4: أي نص في الصفحة
                    if not sms_texts:
                        full_text = soup.get_text()
                        lines = [l.strip() for l in full_text.splitlines() if len(l.strip()) > 15]
                        for line in lines:
                            if any(x in line.lower() for x in ['code', 'otp', 'verify', 'رمز', 'whatsapp', 'telegram']):
...
def fetch_ivasms_messages():
    ...
    try:
        ...
        # تاريخ اليوم الحالي بتوقيت الموقع
        today = datetime.utcnow()
        start_date = today.strftime('%Y-%m-%d')   # صيغة ivasms.com
        end_date = today.strftime('%Y-%m-%d')

        # --- محاولة 1: endpoint الأصلي ---
        ...
        # جرب كل أنواع العناصر الممكنة
        country_groups = (
            summary_soup.find_all('div', {'class': 'pointer'}) or
            summary_soup.find_all(onclick=re.compile(r'getDetials')) or
            summary_soup.select('[onclick*="getDetials"]') or
            summary_soup.select('[onclick*="getDetails"]')
        )

        # --- محاولة 2: صفحة الـ received مباشرة ---
        if not country_groups:
            print(f"[{dash['name']}] 🔄 محاولة جلب الصفحة مباشرة...")
            page_resp = session.get("https://www.ivasms.com/portal/sms/received", headers=headers, timeout=30)
            page_soup = BeautifulSoup(page_resp.text, 'html.parser')

            # تجديد CSRF من الصفحة
            new_csrf = page_soup.find('meta', {'name': 'csrf-token'})
            if new_csrf:
                csrf_token = new_csrf.get('content')
                dash['csrf_token'] = csrf_token

            country_groups = (
                page_soup.find_all('div', {'class': 'pointer'}) or
                page_soup.select('[onclick*="getDetials"]') or
                page_soup.select('[onclick*="getDetails"]') or
                page_soup.find_all(onclick=True)
            )
            summary_soup = page_soup

        if not country_groups:
            # --- محاولة 3: جلب الأرقام المعروفة مباشرة من الكومبو ---
            print(f"[{dash['name']}] 🔄 جلب مباشر للأرقام من الكومبو...")
            return fetch_sms_for_combo_numbers(session, base_url, csrf_token, headers, start_date, end_date)

        # استخراج group_ids
        group_ids = []
        for group in country_groups:
            onclick = group.get('onclick', '')
            match = re.search(r"getDeti(?:a|ai)ls\('(.+?)'\)", onclick, re.IGNORECASE)
            if match:
                group_ids.append(match.group(1))

        if not group_ids:
            print(f"[{dash['name']}] 🔄 ما لاقى group_ids، جلب مباشر...")
            return fetch_sms_for_combo_numbers(session, base_url, csrf_token, headers, start_date, end_date)
        ...
        for group_id in group_ids:
            ...
            number_divs = (
                numbers_soup.select("div[onclick*='getDetialsNumber']") or
                numbers_soup.select("div[onclick*='getDetailsNumber']") or
                numbers_soup.find_all(onclick=re.compile(r'getDetials|getDetails'))
            )
            phone_numbers = [div.text.strip() for div in number_divs if div.text.strip()]
...
def send_otp_to_user_and_group(date_str, number, sms):
    ...
    if user_id:
        try:
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("𝑂𝑊𝑁𝐸𝑅⚙️", url="https://t.me/NXQEX"),
                types.InlineKeyboardButton("𝐾: 𝑆𝑀𝑆", url="https://t.me/X_Y_Px")
            )
            bot.send_message(
                user_id,
    ...
    try:
        ...
        keyboard = {
            "inline_keyboard": [
                ...
                [
                    {"text": "💬 𝑆𝑀𝑆 ↗️", "url": "https://t.me/X_V_Bx"},
                    {"text": "𝐵𝑂𝑇 ↗️", "url": "https://t.me/J_AX_BOT"}
                ],
                
                # الصف الثالث: زر المطور
                [
                    {"text": "M:𝐵𝑂𝑇 ↗️", "url": "https://t.me/NXQEX"}
                ]
            ]
        }
    except Exception as e:
